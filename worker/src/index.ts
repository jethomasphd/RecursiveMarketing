...
// Result of a USAJobs lookup. `error` is populated whenever the call did not
// succeed, so a broken connection is never mistaken for "zero matching jobs".
...
const SCHEDULE_CODES: Record<string, string> = {
...
const RATE_INTERVALS: Record<string, string> = {
...
function pickSchedule(d: any): string {
...
function buildParams(keyword: string, location: string, legacy: boolean): URLSearchParams {
...
async function runSearch(keyword: string, location: string, env: Env, legacy: boolean): Promise<JobSearchResult> {
...
async function searchUSAJobs(keyword: string, location: string, env: Env): Promise<JobSearchResult> {
...
function jobsForClaude(result: JobSearchResult): string {
...
// assuming content[0].
function textFromClaude(data: any): string {
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    let jobResult: JobSearchResult = { items: [], total: 0 };
    let body: any = {};
